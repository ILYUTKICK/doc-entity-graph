"""Очистка извлечённых сущностей и пересборка итогового графа.

Вход:
    Файлы сущностей из Фазы 3 и файлы чанков из Фазы 2.

Выход:
    ``outputs/entity_graph_clean.*``, ``resolved_entities_clean.json`` и
    ``graph_metrics_clean.json``.

Модуль фильтрует шумные SpaCy-сущности, при необходимости обогащает их через
GLiNER, объединяет дубли и экспортирует итоговый граф совместной встречаемости.
"""
